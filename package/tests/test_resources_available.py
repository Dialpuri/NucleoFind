...
def test_files_intact():
    ...
    assert nucleofind_model_folder_path.exists(), (
        "Could not find nucleofind_models folder in CLIBD"
    )

    onnx_model_path = nucleofind_model_folder_path / f"nucleofind-core.onnx"
    assert onnx_model_path.exists(), (
        f"Could not find nucleofind-core.onnx model in nucleofind_models folder"
    )

    onnx_model_sum = calculate_md5_hash(str(onnx_model_path))
    assert onnx_model_sum == data["core"], (
        f"Model checksum mismatch for {onnx_model_path}"
    )

    database_path = nucleofind_model_folder_path / "nucleofind-database.cif"
    assert database_path.exists(), (
        f"Could not find nucleofind-database.cif in nucleofind_models folder"
    )

    database_sum = calculate_md5_hash(str(database_path))
    assert database_sum == data["database"], (
        f"Database checksum mismatch for {database_path}"
    )
