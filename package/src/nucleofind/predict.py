...
class Prediction:
    def __init__(
        ...
        compute_entire_cell: bool = False,
    ):
        ...
        if use_gpu and platform.system() == "Darwin":
            logging.warning(
                "GPU acceleration was specified but is not supported on MacOS, continuing with CPU only"
            )
            self.use_gpu = False
    ...
    def make_prediction(
        self,
        file_path: str,
        column_labels: List[str] = ["FWT", "PHWT"],
    ...
    def save_predicted_map(self, output_path: str):
        for index, grid in enumerate(self.predicted_grids):
            ...
            if ".map" in output_path:
                ccp4.write_ccp4_map(
                    output_path.replace(".map", f"_{self.model_names[index]}.map")
                )
            else:
    ...
    def save_variance_map(self, output_path: str):
        for index, grid in enumerate(self.variance_grids):
            if not self.compute_variance:
                logging.warning(
                    "Attempting to output a variance map without specifying compute_variance=True"
                )
                return
            ccp4 = gemmi.Ccp4Map()
            ccp4.grid = grid
            ...
            if ".map" in output_path:
                ccp4.write_ccp4_map(
                    output_path.replace(
                        ".map", f"_variance_{self.model_names[index]}.map"
                    )
                )
            else:
                ccp4.write_ccp4_map(
                    f"{output_path}_variance_{self.model_names[index]}.map"
                )
    ...
    def _load_model(self):
        ...
        self.models = [
            rt.InferenceSession(model, providers=providers, sess_options=sess_options)
    ...
    def _load_mtz(
        ...
        column_names: List[str] = ["FWT", "PHWT"],
    ):
        ...
        self.raw_grid = mtz.transform_f_phi_to_map(
            *column_names, sample_rate=sample_rate
        )
        self.raw_grid.normalize()
        if resolution_cutoff:
    ...
    def _get_bounding_box(self, grid: gemmi.FloatGrid) -> gemmi.PositionBox:
        ...
        corners = [
            ...
            for fractional in (
                extent.minimum,
                gemmi.Fractional(
                    extent.maximum[0], extent.minimum[1], extent.minimum[2]
                ),
                gemmi.Fractional(
                    extent.minimum[0], extent.maximum[1], extent.minimum[2]
                ),
                gemmi.Fractional(
                    extent.minimum[0], extent.minimum[1], extent.maximum[2]
                ),
                gemmi.Fractional(
                    extent.maximum[0], extent.maximum[1], extent.minimum[2]
                ),
                gemmi.Fractional(
                    extent.maximum[0], extent.minimum[1], extent.maximum[2]
                ),
                gemmi.Fractional(
                    extent.minimum[0], extent.maximum[1], extent.maximum[2]
                ),
                extent.maximum,
            )
        ]
    ...
    def _reinterpolate_to_output(
        self, grid_to_interp: np.ndarray
    ) -> List[gemmi.FloatGrid]:
    ...
    def _calculate_translations(self, overlap: int = 32):
        ...
        logging.debug(f"Interpolated grid shape : {self.interpolated_grid.array.shape}")
    ...
    def _predict(self, raw_values: bool = False, overlap: int = 32):
        ...
        for translation in tqdm(
            ...
            total=len(self.translation_list),
            ...
            if np.sum(sub_array) == 0:
                predicted_map[x : x + 32, y : y + 32, z : z + 32, :] += np.zeros(
                    32, 32, 32
                )
                    ...
                    for i, a in enumerate(range(x, x + 32)):
                        for j, b in enumerate(range(y, y + 32)):
                            for k, c in enumerate(range(z, z + 32)):
                                variance_array_map[a, b, c, model_index].append(
                                    arg_max[i, j, k]
                                )
        ...
        predicted_map = predicted_map[
            0 : (32 * self.na), 0 : (32 * self.nb), 0 : (32 * self.nc), :
        ]
            ...
            for i in range(variance_maps.shape[0]):
                for j in range(variance_maps.shape[1]):
                    for k in range(variance_maps.shape[2]):
                        for model_index in range(len(self.models)):
                            variance_maps[i, j, k, model_index] = np.var(
                                variance_array_map[i, j, k, model_index]
                            )
...
def run():
    ...
    parser.add_argument(
        "-silent", action=argparse.BooleanOptionalAction, help="Turn off progress bar"
    )
    ...
    if not model_paths:
        if not args["m"] or not os.path.exists(args["m"]):
            raise FileNotFoundError(
                "Model path could not be found, check the supplied path"
            )
        # model_paths = args["m"]
    else:
    ...
    logging.info(
        f"Time taken {end - start:.2f} seconds / {(end - start) / 60:.2f} minutes"
    )
...
to install a single model (choose either phosphate, sugar or base)
    ...
    print(
        f"The specified model type '{model_selection}' could not be found, please add one of the following flags"
    )
