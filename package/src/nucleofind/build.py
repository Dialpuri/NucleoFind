...
def find_database() -> str:
    ...
    if not clibd:
        raise RuntimeError(
            "Could not find the NucleoFind database in site-packages or the CCP4 installation."
    ...
    raise RuntimeError(
        "Could not find the NucleoFind database. Please install NucleoFind using the install command."
    )
